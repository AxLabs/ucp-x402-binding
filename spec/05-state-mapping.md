# State Mapping: UCP Checkout Session States vs x402 Settlement
...
x402 settlement lifecycle: challenge issued -> signature submitted -> gateway verifies (amount, payTo, validity window, nonce) -> on-chain settlement -> receipt.
...
| Agent submits `complete` with PAYMENT-SIGNATURE | Same-URL: `complete_in_progress`; External-URL: treated as reconcile (§3.3.3) | Same-URL: signature received, verification underway. External-URL: the merchant MUST NOT replay the signature to `resource.url`; it polls settlement / consumes the upstream fulfill, then transitions as below |
...
| External-URL: agent pays `resource.url` directly (outside UCP) | no status change | Out-of-protocol settlement. UCP observes only the consequence: the following `complete` finds the settlement and transitions |
| External-URL: agent POSTs `complete` again after paying | `completed` if settlement observed, else `complete_in_progress` or stays `ready_for_complete` | Reconcile call (empty body or same instrument selection). Merchant polls settlement state; `payment_failed` stays a recoverable message while settlement may still land |
| Gateway verifies + settles on-chain | `completed` | Order webhooks fire as usual |
