...
4. ~~Conditional asset-shape rules: the schema currently knows `eip155` (0x+40 hex) and `solana` (base58 32-44). Each new x402 network binding should contribute its rule when adopted.~~ **RESOLVED**: per-network schemas referenced via `x402.networkSchemas` (see above). The core schema stays chain-agnostic; asset-shape rules live in `schema/networks/<namespace>.schema.json`, maintained with each chain community. A chain can register its schema by hosting it and adding the URL to its handler entries.
5. ~~`bip122` assets: whether `asset` is the genesis hash, empty, or omitted for native assets.~~ **RESOLVED (design direction)**: delegated to the per-network schema. A `bip122` schema would define the native-asset convention (e.g. `asset` omitted or empty for the native asset, or a fixed identifier), and the direction is that native assets use empty or omitted `asset` with `decimals: 0`. The core schema accepts both, and the decision ships with the first bip122 per-network schema rather than in the core spec.
